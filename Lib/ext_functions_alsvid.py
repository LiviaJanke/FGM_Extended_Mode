...
sys.path.append('/home/lme19/Lib')

sys.path.append('/cluster/data/raw/')
...
def get_calibrated_ext_data(index, craft):
    ...
    ext_entry = Ext_entry_times[index]

    next_ext_entry = Ext_entry_times[index + 1]


    if index > 1:
        prev_ext_entry = Ext_entry_times[index - 1]
        
        prev_ext_exit = closest_higher_date(Ext_exit_times, prev_ext_entry)
        
        prev_MSA_dump =  closest_higher_date(MSA_dumps, prev_ext_exit)

    ext_exit = closest_higher_date(Ext_exit_times, ext_entry)

    next_ext_exit = closest_higher_date(Ext_exit_times, next_ext_entry)
